...
import Background from "../../../app/scripts/ts/background/background.ts"
...
describe('Yahoo!オークションAPI', () => {
    var bk;

    describe('アプリケーションID正常取得', () => {
        beforeEach(() => {
            bk = new Background();
...
describe('商品データ取得', () => {
    ...
    it('正常取得', () => {
        spyOn(bk, 'execYahooAuctionAPI').and.callFake(() => {
            var def = $.Deferred();
            def.resolve({
        ...
        bk.getSyohinData('auctionID', (res) => {
            expect(res).toEqual({
                result: true,
                data: {
                    syohinSetsumei: '商品説明',
